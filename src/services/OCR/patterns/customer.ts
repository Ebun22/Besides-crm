import type { customer_data } from "../types/OCR_results";
...
export function extractCustomer(text: string): customer_data {
  ...
  data.supply_holder   = text.match(SUPPLY_HOLDER_RE)?.[1]?.trim();
  data.tax_code        = text.match(TAX_CODE_RE)?.[1]?.trim();
  data.mailing_address = text.match(RESIDENTIAL_ADDRESS_RE)?.[1]?.trim();
  data.supply_address  = text.match(SUPPLY_ADDRESS_RE)?.[1]?.trim();
