...
import { electric_data } from '../types/OCR_results.ts';

const INVOICE_TYPE_LABEL_RE       = /Numero\s+fattura\s+elettronica/i;
const POD_RE                      = /Codice\s+POD:?\s*(IT\d{3}E\d{8})/i;
...
const VOLTAGE_LEVEL_RE            = /(?:Valore di tensione nominale per alimentazione|Tensione di fornitura|Livello di tensione|Livello di consegna)\s*:?\s*((?:BASSA|MEDIA|ALTA)\s+TENSIONE)/i;
const INTENDED_USE_RE             = /Tipologia cliente:?\s*([A-ZÀ-Ý]{2,}(?:\s+[A-ZÀ-Ý]{2,})*)/;
const ANNUAL_CONSUMPTION_RE       = /Consumo annuo:?\s*([\d.,]+)\s*kWh/i;
const TOTAL_ACTIVE_CONSUMPTION_RE = /TOTALI\s+CONSUMI\s+ENERGIA\s+ATTIVA\s+([\d.,]+)/i;
...
export function extract_electric(text : string) : electric_data {
  ...
  data.invoice_type = INVOICE_TYPE_LABEL_RE.test(text) ? 'Electronic' : 'Paper';
